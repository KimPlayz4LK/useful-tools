...
if WScript.Arguments.Length=1 and Right(WScript.Arguments(0),4)=".txt" then
...
set oneFile=fso.OpenTextFile(fso.GetParentFolderName(WScript.ScriptFullName)&"\packedFiles.txt",2,True)
...
x=msgbox("No files detected, try drag'n'dropping files you want to pack or the .TXT file to unpack.",0,"DotOne")
end if